	...
	<div class="row-fluid">
			...
			<table border='0'>
				<tr>
					...
					<td><input class="span10 date-picker" name="publish_date" value="${pd.publish_date}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:100px;" placeholder="出版日期从"/></td>
					<td><input class="span10 date-picker" name="update_date" value="${pd.update_date}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:100px;" placeholder="出版日期至"/></td>
					...
					<c:when test="${not empty newsList}">
						...
						<c:forEach items="${newsList}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td>${fn:substring(var.introduction ,0,10)} ...</td>
								<td>${var.content }</td>
								<%-- <td>
								<a href="<%=basePath%>TP/${var.pic}" title="${var.name}" class="bwGal"><img src="<%=basePath%>TP/${var.pic}" alt="${var.name}" width="100"></a>
								</td> --%>
								<td>
								<fmt:formatDate value="${var.publish_date}" pattern="yyyy-MM-dd" />
								</td>
								<td>
								<fmt:formatDate value="${var.update_date }" pattern="yyyy-MM-dd" />
								</td>
