...
import com.huixin.haidong.framework.utils.AppUtil;
import com.huixin.haidong.framework.utils.Const;
import com.huixin.haidong.framework.utils.ObjectExcelView;
import com.huixin.haidong.framework.utils.PageData;
import com.huixin.haidong.system.entity.Page;
...
public class ${objectName}Controller extends BaseController {
	...
	public ModelAndView save(Session session) throws Exception{
		logBefore(logger, "新增${objectName}");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		String user = session.getAttribute(Const.SESSION_USERNAME).toString();
		pd = this.getPageData();
		pd.put("id", this.get32UUID());	//主键
		pd.put("create_user", user);
<#list fieldList as var>
	...
	public ModelAndView edit(Session session) throws Exception{
		logBefore(logger, "修改${objectName}");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		String user = session.getAttribute(Const.SESSION_USERNAME).toString();
		pd.put("modify_user", user);
		pd.put("modify_time", DateUtil.getTime().toString());
