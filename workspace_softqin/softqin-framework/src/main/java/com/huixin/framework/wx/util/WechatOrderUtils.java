...
import com.huixin.framework.wx.beans.WechatOrder;
...
public class WechatOrderUtils {
	...
	protected static Logger logger = Logger.getLogger(WechatOrderUtils.class);
     ...
     */
    public static synchronized JSONObject createOrder(String detail, String desc, String openid, String ip, String goodSn, String orderSn, String amount, String type,String backtype) {
        ...
        if (StringUtils.isBlank(detail) || StringUtils.isBlank(desc) || StringUtils.isBlank(ip)
        	...
        	logger.error("微信支付统一下单请求错误：请求参数不足");
        ...
        try {//进行格式转换异常获取，保证数目正确
        	...
        	logger.error("微信支付统一下单请求错误：请求金额格式错误", e);
            result.put("status", "error");
        ...
        if (relAmount == 0) {//微信支付的支付金额必须为大于0的int类型，单位为分
            logger.error("微信支付统一下单请求错误：请求金额不能为0", null);
            result.put("status", "error");
        ...
        if (!("JSAPI".equalsIgnoreCase(type) || "NATIVE".equalsIgnoreCase(type) || "APP".equalsIgnoreCase(type))) {
            logger.error("微信支付统一下单请求错误：支付类型为空", null);
            result.put("status", "error");
       ...
       /* 公众号调起微信支付的时候，必须要有openID*/
        if ("JSAPI".equalsIgnoreCase(type) && StringUtils.isBlank(openid)) {
            logger.error("微信支付统一下单请求错误：请求参数不足,openid:" + openid, null);
            result.put("status", "error");
            result.put("msg", "请求参数不足");
            result.put("obj", null);
            return result;
        ...
        String wx_callback="";
        wx_callback  = PropertiesUtil.getValue("wechat.properties", "wx_callback");// 获取微信支付回调接口
        if("wxchongzhi".equals(backtype)){
        	 wx_callback  = PropertiesUtil.getValue("wechat.properties", "wxmoney_callback");// 获取微信公众号支付回调接口
        }
        else if("shopweixinpay".equals(backtype)) {
        	 wx_callback  = PropertiesUtil.getValue("wechat.properties", "wxshop_callback");// 获取微信支付商城回调接口
        }
        String wx_key = "";//微信商户后台设置的key
        if("APP".equalsIgnoreCase(type)) {
        	wx_key = PropertiesUtil.getValue("wechat.properties", "app_api_key");
        } else {
        	wx_key = PropertiesUtil.getValue("wechat.properties", "wx_key");
        }
        ...
        if (StringUtils.isBlank(wx_order) || StringUtils.isBlank(mchappid)
            ...
            logger.error("微信支付统一下单请求错误：系统配置信息缺失", null);
            result.put("status", "error");
        ...
        String nonceStr1 = HttpUtils.getNonce_str();
        // 发送报文模板,其中部分字段是可选字段
        String xml = "" +
                "<xml>" +
        ...
        xml = xml.replace("1add1a30ac87aa2db72f57a2375d8fec", nonceStr1);
        ...
        try {//注意，此处的httputil一定发送请求的时候一定要注意中文乱码问题，中文乱码问题会导致在客户端加密是正确的，可是微信端返回的是加密错误
            ...
            logger.error("微信支付统一下单失败:http请求失败", e);
            result.put("status", "error");
        ...
        if ("SUCCESS".equals(order.getReturn_code()) && "SUCCESS".equals(order.getResult_code())) {
            logger.error("微信支付统一下单请求成功：" + order.getPrepay_id(), null);
        } else {
            logger.error("微信支付统一下单请求错误：" + order.getReturn_msg() + order.getErr_code(), null);
            result.put("status", "error");
            result.put("msg", "http请求失败");
            result.put("obj", null);
            return result;
        ...
        if ("JSAPI".equalsIgnoreCase(type)) {
            ...
            jsonObject.put("nonceStr", nonceStr1);
